...
async def recommend_command_from_callback(query, context: ContextTypes.DEFAULT_TYPE):
...
async def genre_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['current_recommendations'] = extract_item_names(recommendations)
...
def extract_item_names(recommendations: str) -> list:
    ...
    return items[:3]  # Return max 3 items"""Recommendation command handler."""
import logging
import os
import re
from telegram import Update
...
def extract_item_names(recommendations: str) -> list:
    ...
    return items[:3]  # Return max 3 items